...
class Planner:
    def __init__(self,config,device='cpu'):
        ...
        self.continuations={int(p):e.load(ROOT/path) for p,path in config.get('continuation_by_players',{}).items()}
        allowed={'worlds','refine_worlds','refine_margin','pair_worlds','pair_topk','prior_weight','prior_from_continuation','bullhead_weight','rank_weight','endgame_multiplier',
            'stochastic_proxy','proxy_temperature','belief_oversample','belief_history','belief_strength',
            'belief_epsilon','belief_temperature','belief_min_ess','belief_prior_mix'}
        for override in config.get('player_overrides',{}).values():
            if set(override)-allowed:raise ValueError('Player overrides may only change search settings, not model loading')
        ...
        if config.get('engine')=='torch':
            ...
            if config.get('cuda_graphs'):name+='_graphs'
            if name not in sys.modules:
                spec=importlib.util.spec_from_file_location(name,path);module=importlib.util.module_from_spec(spec)
                sys.modules[name]=module;spec.loader.exec_module(module)
            self.torch_env=sys.modules[name]
            if config.get('cuda_graphs'):
                graph_path=Path(__file__).with_name('progressive_cuda_graphs.py')
                graph_name='graph_runtime_'+hashlib.sha256(graph_path.read_bytes()).hexdigest()[:16]
                if graph_name not in sys.modules:
                    spec=importlib.util.spec_from_file_location(graph_name,graph_path);module=importlib.util.module_from_spec(spec)
                    sys.modules[graph_name]=module;spec.loader.exec_module(module)
                self.graph_stats=sys.modules[graph_name].install(self.torch_env,max_batch=int(config.get('graph_max_batch',32768)))
        if config.get('belief_oversample') or any(v.get('belief_oversample') for v in config.get('player_overrides',{}).values()):
    ...
    def choose(self,observation):
        ...
        original=self.config;override=original.get('player_overrides',{}).get(str(observation['playerCount']))
        if override:self.config=dict(original,**override)
        try:
            result=self.choose_configured(observation)
            if self.config.get('pair_worlds') and len(observation['hand'])>2:
                assert self.config.get('engine')=='torch','Two-card plans require the Torch engine'
                count=min(int(self.config.get('pair_topk',3)),len(result['candidates']))
                shortlist=[result['candidates'][i] for i in np.argsort(result['utility'])[:count]]
                self.config=dict(self.config,worlds=int(self.config['pair_worlds']),pair_card_subset=shortlist)
                result=self.choose_torch(observation)
            target=int(self.config.get('refine_worlds',0))
            if target>int(self.config.get('worlds',32)) and len(result.get('utility',[]))>1:
                ordered=np.partition(np.asarray(result['utility']),1);margin=float(ordered[1]-ordered[0])
                if margin<float(self.config.get('refine_margin',.04)):
                    before=result['worlds'];self.config=dict(self.config,worlds=target)
                    result=self.choose_configured(observation)
                    result['refinement']=dict(initial_worlds=before,initial_margin=margin,final_worlds=result['worlds'])
            result['seconds']=time.perf_counter()-started
            return result
        finally:self.config=original

    def choose_configured(self,observation):
        ...
        cfg=self.config;root=public_game(observation);continuation=self.continuations.get(root.players,self.continuation)
        ...
        prior=self.utilities(continuation if cfg.get('prior_from_continuation') else self.base,permutations).mean(axis=0)
        ...
        for turn in range(h):
            ...
            if turn==0:
                ...
                actions[:,0]=self.utilities(continuation,features[:,0]).argmin(axis=-1)
    ...
    def choose_torch(self,observation):
        started=time.perf_counter();hand=sorted(observation['hand']);h=len(hand)
        if h==1:return dict(card=hand[0],seconds=time.perf_counter()-started)
        cfg=self.config;root=public_game(observation);p=root.players;continuation=self.continuations.get(p,self.continuation)
        f=np.array([[encode_candidate(observation,c) for c in hand]],dtype=np.float32)
        prior=self.utilities(continuation if cfg.get('prior_from_continuation') else self.base,
            np.concatenate([e.permute_features(f,perm) for perm in e.PERMS])).mean(axis=0)
        worlds=int(cfg.get('worlds',32));seed=int(observation.get('seed',1))+int(cfg.get('seed_offset',600013))
        if cfg.get('endgame_multiplier') and h<=3:worlds*=int(cfg['endgame_multiplier'])
        proposals=worlds*max(1,int(cfg.get('belief_oversample',1))) if observation.get('history') else worlds
        ...
        pair_mode=bool(cfg.get('pair_card_subset'))
        root_cards=sorted(cfg['pair_card_subset']) if pair_mode else hand
        assert set(root_cards).issubset(hand) and len(root_cards)==len(set(root_cards))
        plans=[(first,second) for first in root_cards for second in hand if first!=second] if pair_mode else [(first,None) for first in hand]
        branches=len(plans)
        root_groups=groups(owners);all_groups=groups(np.tile(owners,(branches,1)))
        initial=self.torch_env.TorchGame(sampled,self.device);game=initial.repeat(branches);initial_scores=game.scores.clone()
        def choose_actions(table,indices,uniforms,focal):
            ...
            if focal:choice[:,0]=self.torch_env.split_utilities(continuation,common[:,0],action[:,0]).argmin(dim=-1)
            return choice
        immediate=None
        for turn in range(h):
            ...
            if turn==0:
                ...
                cards=initial.hands.gather(2,actions[:,:,None]).squeeze(-1).repeat(branches,1)
                cards[:,0]=torch.tensor([plan[0] for plan in plans],device=self.device).repeat_interleave(worlds)
            else:
                actions=choose_actions(game,all_groups,uniforms.repeat(branches,1),True)
                cards=game.hands.gather(2,actions[:,:,None]).squeeze(-1)
                if pair_mode and turn==1:
                    cards[:,0]=torch.tensor([plan[1] for plan in plans],device=self.device).repeat_interleave(worlds)
            costs=game.step(cards)
        ...
        plan_values=reward.reshape(branches,worlds).mean(axis=1)
        immediate_values=immediate.cpu().numpy().reshape(branches,worlds).mean(axis=1)
        values=plan_values.reshape(len(root_cards),h-1).max(axis=1) if pair_mode else plan_values
        prior_selected=prior[[hand.index(card) for card in root_cards]]
        utility=-values+float(cfg.get('prior_weight',0))*(prior_selected-prior.min())
        if pair_mode:immediate_values=immediate_values.reshape(len(root_cards),h-1).mean(axis=1)
        result=dict(card=root_cards[int(utility.argmin())],candidates=root_cards,values=values.tolist(),utility=utility.tolist(),worlds=worlds,
            mean_immediate=immediate_values.tolist())
        if pair_mode:
            plan_utility=-plan_values+float(cfg.get('prior_weight',0))*np.repeat(prior_selected-prior.min(),h-1)
            result['two_card_plan']=dict(branches=branches,shortlist=root_cards,
                selected_sequence=list(plans[int(plan_utility.argmin())]))
        result['seconds']=time.perf_counter()-started
