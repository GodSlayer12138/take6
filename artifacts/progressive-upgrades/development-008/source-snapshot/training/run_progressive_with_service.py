"""Run a fresh frozen campaign manifest using a shared GPU service."""
import argparse
import json
import os
from pathlib import Path
# Every local opponent explicitly runs on CPU. Only the separate service owns CUDA;
# hide it before importing modules that probe CUDA availability in each worker.
...
def main(args):
    ...
    execution=dict(kind='single-gpu-service',streams=service.get('streams',1),client_cuda_visible_devices='-1',sources={name:campaign.sha(campaign.ROOT/name) for name in SOURCES},
