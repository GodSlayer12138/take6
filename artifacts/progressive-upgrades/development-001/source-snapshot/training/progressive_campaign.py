...
def prepare_run(name,configs,deals,purpose,reference=None):
    ...
    sources=['training/progressive_planner.py','training/progressive_campaign.py','training/explore_small_strategies.py',
...
def main():
    ...
    parser=argparse.ArgumentParser();parser.add_argument('--manifest');parser.add_argument('--pilot',action='store_true');parser.add_argument('--workers',type=int,default=8);args=parser.parse_args()
    initialize_campaign();path=Path(args.manifest) if args.manifest else pilot()
