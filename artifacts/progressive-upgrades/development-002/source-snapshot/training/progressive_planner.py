...
class Planner:
    ...
    def choose_torch(self,observation):
        ...
        worlds=int(cfg.get('worlds',32));seed=int(observation.get('seed',1))+int(cfg.get('seed_offset',600013))
        if cfg.get('endgame_multiplier') and h<=3:worlds*=int(cfg['endgame_multiplier'])
        sampled=e.public_worlds(root,0,worlds,seed);rng=np.random.default_rng(seed+11);profile=cfg.get('opponents','mixed')
        if profile=='neural':owners=rng.integers(0,len(self.pool),(worlds,p))
        elif profile=='base':owners=np.zeros((worlds,p),dtype=np.int64)
        elif profile=='actual-proxy':
            owners=np.zeros((worlds,p),dtype=np.int64)
            for i in range(worlds):owners[i,1:]=rng.choice(8,p-1,replace=False)
        else:owners=rng.integers(0,len(self.pool)+4,(worlds,p))
        owners[:,0]=-1
