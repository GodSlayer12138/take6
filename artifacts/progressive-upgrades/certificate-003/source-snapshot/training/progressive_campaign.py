...
def prepare_run(name,configs,deals,purpose,reference=None):
    ...
    sources=['training/progressive_planner.py','training/progressive_torch_env.py','training/progressive_belief.py','training/progressive_cuda_graphs.py',
        'training/progressive_inference_service.py','training/run_progressive_with_service.py','training/run_progressive_inline.py','training/progressive_campaign.py','training/explore_small_strategies.py',
    ...
    for candidate,params in configs.items():
        ...
        dependencies.extend(ROOT/path for path in params.get('continuation_by_players',{}).values())
        if params.get('engine')=='torch':dependencies.append(snapshot/'training/progressive_torch_env.py')
        if params.get('cuda_graphs'):dependencies.append(snapshot/'training/progressive_cuda_graphs.py')
        if params.get('belief_oversample') or any(v.get('belief_oversample') for v in params.get('player_overrides',{}).values()):
            dependencies.append(snapshot/'training/progressive_belief.py')
    ...
    # This historical league is now used by the second opponent-proxy training run.
    for line in (ROOT/'artifacts/local-tournament/classic4/blocks.jsonl').read_text().splitlines():
        used.update(r['deal_seed'] for r in json.loads(line))
